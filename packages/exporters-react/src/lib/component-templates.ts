...
function generateDataTable(name: string): string {
  return joinLines([
    ...
    `import { isNumericFieldKey, TABLE_NUMERIC_CELL_CLASS } from '../presentation/numeric-fields';`,
    ``,
    `export interface ${name}Props {`,
    `  id?: string;`,
    `  data: Row[];`,
    ...
    `          <tr>`,
    `            {columns.map((column) => (`,
    `              <th key={column} className={isNumericFieldKey(column) ? TABLE_NUMERIC_CELL_CLASS : undefined}>`,
    `                {column}`,
    `              </th>`,
    `            ))}`,
    `          </tr>`,
    ...
    `            >`,
    `              {columns.map((column) => (`,
    `                <td key={column} className={isNumericFieldKey(column) ? TABLE_NUMERIC_CELL_CLASS : undefined}>`,
    `                  {String(row[column] ?? '')}`,
    `                </td>`,
    `              ))}`,
    `            </tr>`,
...
function generateDetailPanel(name: string): string {
  return joinLines([
    ...
    ``,
    `import type { Row } from '../types';`,
    `import { DETAIL_NUMERIC_VALUE_CLASS, isNumericFieldKey } from '../presentation/numeric-fields';`,
    ``,
    `export interface ${name}Props {`,
    `  id?: string;`,
    `  title?: string;`,
    ...
    `              <dt>{key}</dt>`,
    `              <dd className={isNumericFieldKey(key) ? DETAIL_NUMERIC_VALUE_CLASS : undefined}>`,
    `                {String(value ?? '')}`,
    `              </dd>`,
    `            </div>`,
    `          ))}`,
...
function generateSvgInline(name: string): string {
  return joinLines([
    `'use client';`,
    ``,
    `import type { Row } from '../types';`,
    ...
    `      >`,
    `        <img src={url} alt={ariaLabel} style={{ width: '100%', height: '100%', objectFit: 'contain' }} />`,
    `      </figure>`,
    `    );`,
    `  }`,
    ``,
    `  if (sourceMode === 'path' && assetPath) {`,
    `    return (`,
    `      <figure className="svg-inline svg-inline--asset" id={id} style={{ width, height }} aria-label={ariaLabel}>`,
    `        <p className="svg-inline__placeholder">Asset: {assetPath}</p>`,
    `      </figure>`,
    `    );`,
    `  }`,
    ``,
    ...
    `      className="svg-inline"`,
    `      id={id}`,
    `      style={{ width, height, color: fillColor }}`,
    `      aria-label={ariaLabel}`,
...
function generateSvgIcon(name: string): string {
  return joinLines([
    `'use client';`,
    ``,
    `import type { Row } from '../types';`,
