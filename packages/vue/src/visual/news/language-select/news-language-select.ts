...
/** @rosettadash/vue/visual/news/language-select — visual.news.language-select */
export const NewsLanguageSelect = defineComponent({
  name: 'RdNewsLanguageSelect',
  props: {
  ...
  setup(props, { slots, attrs }) {
    return () => {
      const rootClass = ['rd-news-language-select', props.className, typeof attrs.class === 'string' ? attrs.class : ''].filter(Boolean).join(' ');
      return h('section', { class: rootClass, 'data-testid': 'rd-news-language-select' }, [
...
export type NewsLanguageSelectComponent = typeof NewsLanguageSelect;
