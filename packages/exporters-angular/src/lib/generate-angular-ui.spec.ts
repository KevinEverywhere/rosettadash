...
describe('generateAngularUiFiles', () => {
  ...
  it('generates Angular three.js bar chart stub', () => {
    ...
    const files = generateAngularUiFiles(ir);
    const chartFile = files.find((file) => file.content.includes('OrbitControls'));
    expect(chartFile?.content).toContain('THREE.BoxGeometry');
  ...
  it('generates Angular three.js geo globe stub', () => {
    ...
    expect(globeFile?.content).toContain('TextureLoader');
