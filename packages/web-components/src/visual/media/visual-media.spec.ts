...
} from './equirect-viewport/rd-equirect-viewport.js';
import {
  RdVideoSourceElement,
  DB_VIDEO_SOURCE_TAG,
  registerRdVideoSource,
} from './video-source/rd-video-source.js';

describe('@rosettadash/web-components/visual/media', () => {
  beforeAll(() => {
    registerRdVideoSource();
    registerRdEquirectViewport();
  });
  ...
  it('emits crop-region with flat-crop filter defaults', async () => {
  ...
  it('renders rectilinear preview mode with v360 filter', async () => {
    const viewport = document.createElement(DB_EQUIRECT_VIEWPORT_TAG) as RdEquirectViewportElement;
    document.body.appendChild(viewport);
    await viewport.whenReady();
